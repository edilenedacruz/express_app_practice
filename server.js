...
app.post('/api/v1/footnotes', (request, response) => {
  ...
  for (let requiredParameter of ['note', 'paper_id']) {
    if (!footnote[requiredParameter]) {
      return response
        .status(422)
        .send({ error: `Expected format: { note: <String>, paper_id: <String> You're missig a ${requiredParameter} property.` });
    }
  }
  ...
  database('footnotes')
    ...
    .then(footnotes => {
      if (footnotes.length) {
        ...
        response.status(404).json({
          error: `Could not find footnotes for paper with id ${requet.params.id}`
        });
      }
    })
    .catch(error => {
